...
class PermutedDKT(nn.Module):
    ...
    def forward(self, concept_input, labels):
        ...
        input = torch.zeros(T, B, self.n_concepts).to(device)
...
def main():
    dkt = PermutedDKT(n_concepts=5).to(device)
    concept_input = torch.randint(0, 5, (4, 2)).to(device)
    print(concept_input)
    labels = (torch.randint(0, 2, (4, 2)) * 2 - 1).to(device)
    print(labels)
